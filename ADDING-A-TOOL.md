...
7. manifest `public/manifests/<id>.webmanifest`（既存をコピーして name / short_name / description / start_url を変更）
   ※ `public/sw.js`（PWA）は**ランタイムキャッシュのみ＝ツール追加での更新は不要**（触らない）
...
- **フォント**：見出し＝**Shippori Mincho B1**（`font-serif`。※無印の"Shippori Mincho"とは別フォント。importは `Shippori_Mincho_B1`）、本文＝Noto Sans JP、数字は `tnum`（等幅）。手書きアクセント＝Yomogi（`font-hand`）。
