// 領収書プレビュー(印刷=この内容がそのままA4に出る)
// 移植元: kantan-chouhyo/app/components/preview/ReceiptPreview.tsx
...
const gridCols = "grid-cols-[1fr_minmax(3.5rem,auto)_minmax(6rem,auto)]";

/** 但し書き(手動編集 or 明細から自動生成) */
export function buildReceiptPurpose(data: FormData): string {
  ...
  return (
    <div className="doc-root min-w-[480px] max-w-none bg-white text-sm leading-7 text-black">
      ...
      <div className="doc-section mb-4 text-center">
        <h1 className="mb-3 font-sans text-3xl font-light tracking-wider">領収書</h1>
      ...
      {/* 領収金額 - 中央に大きく表示 */}
      <div className="doc-section mb-6 text-center">
        <div className="mb-2 text-sm">下記の通り領収いたしました</div>
        <div className="text-xl font-bold">金　¥{formatCurrency(grandTotal)}　也</div>
      </div>
      ...
      {(data?.items || []).length > 0 && (
        <div className="mb-8">
          {/* ヘッダー */}
          <div
            className={`grid ${gridCols} text-[12px] font-semibold tracking-wide text-slate-700`}
          >
            <div className="doc-rule-b border-b border-slate-300 px-2 py-2">内容</div>
      ...
      <div className="doc-section mt-6">
        <div className="mb-1 font-medium">発行元</div>
