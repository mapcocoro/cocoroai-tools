// 請求書プレビュー(印刷=この内容がそのままA4に出る)
// 移植元: kantan-chouhyo/app/components/preview/InvoicePreview.tsx
  ...
  return (
    <div className="doc-root min-w-[480px] max-w-none bg-white text-sm leading-7 text-black">
      ...
      <div className="doc-section mb-4 text-center">
        <h1 className="mb-3 font-sans text-3xl font-light tracking-wider">請求書</h1>
      ...
      <div className="doc-section mb-6">
        <div className="doc-rule-b flex items-end justify-between border-b border-slate-400 pb-1">
          <span className="text-sm">ご請求金額（税込）</span>
          <span className="tnum text-xl font-bold">¥{formatCurrency(grandTotal)}</span>
        </div>
        {data?.paymentSite && (
          <div className="mt-1 text-xs text-slate-500">お支払条件：{data.paymentSite}</div>
        )}
      </div>
